...
export default function GetThingDone() {
    ...
    return (
        <Flex w={"full"} zIndex={"20"} flexDir={"column"} color={"white"} gap={["4", "4", "8"]} pl={["6", "6", "16"]} py={["6", "6", "14"]} pos={"relative"} >
            ...
            <Flex className="hide-scrollbar" ref={ref} w={"full"} zIndex={"20"} overflowX={"auto"} h={"fit-content"} gap={"3"} scrollBehavior={"smooth"} >
                <Flex w={"auto"} gap={"3"} >
                    {data?.map((item) => {
                        return (
            ...
            <Flex w={"full"} justifyContent={"end"} gap={"4"} px={"12"} zIndex={"20"} >
                <Flex onClick={() => scroll(-400)} cursor={"pointer"} w={"40px"} h={"40px"} pr={"1"} justifyContent={"center"} alignItems={"center"} rounded={"full"} borderWidth={"1px"} borderColor={"white"} >
            ...
            <CustomButton onClick={() => push("/auth?create=services")} text={"Browse Professionals"} px={"5"} width={"fit-content"} fontSize={"14px"} mt={"3"} pos={"relative"} zIndex={"20"} borderRadius={"999px"} />
        </Flex>
    )
}